...
contract RPSTournament {
    ...
    event RoundPlayed(address indexed player, uint256 round, Move playerMove, Move computerMove, string result);
    event GameCompleted(
        address indexed player, string result, uint256 playerWins, uint256 computerWins, uint256 payout
    );
    ...
    function playRound(Move playerMove) external {
        ...
        Move computerMove = Move(computerAI.get_move(game.currentRound, game.gameSeed, game.playerMoves));
    ...
    function _completeGame(Game storage game) private {
        ...
        if (game.playerWins > game.computerWins) {
            ...
            // Computer wins - send winnings to fund manager with actual ETH
            result = "Computer Champion!";
            fundManager.addWinnings{value: game.totalPot}(game.totalPot);
        } else {
    ...
    function getGameStatus(address player)
        external
        view
        returns (
            GameState state,
    ...
    function getMoveHistory(address player)
        external
        view
